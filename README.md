...
- `npm run dev` keeps the frontend on `/api` and proxies those requests to the deployed `siteApi` by default; set `VITE_USE_FUNCTIONS_EMULATOR=true` to point it at `127.0.0.1:5001` while running `npm run emulators`.
